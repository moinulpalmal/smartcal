...
@section('content')
    ...
    <div class="page page-profile">
        ...
        <h2>Elastic <span> Booking Details</span></h2>
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="{{route('elastic.booking.recent')}}"> Recent Bookings</a>
                </li>
                <li>
                    <a href="{{route('elastic.booking.detail', ['id', $purchaseOrder->id ])}}"> LPD PO NO: {{$purchaseOrder->lpd_po_no}}</a>
                </li>
            </ul>
        </div>
    </div>
                            ...
                            <div class="mt-10">
                                ...
                                @else
                                    ...
                                    @if($purchaseOrder->status == 'A')
                                        @if(Auth::user()->hasTaskPermission('updateemaster', Auth::user()->id))
                                            <a title="Edit Booking Master Update" class ="myIcon icon-warning icon-ef-3 icon-ef-3b icon-color" href="{{route('elastic.booking.edit', ['id' => $purchaseOrder->id])}}" {{--data-toggle="modal" data-target="#POUpdateModal" --}}data-options="splash-2 splash-ef-12">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                        @endif
                                        @if(Auth::user()->hasTaskPermission('revisee', Auth::user()->id))
                                        ...
                                        @endif
                                        @if(Auth::user()->hasTaskPermission('deletee', Auth::user()->id))
                                    ...
                                    @else
                                        ...
                                        @endif
                                    @endif
                                    <a title="PDF View" class ="myIcon icon-danger icon-ef-3 icon-ef-3b icon-color" {{--target="_blank"--}} href="{{route('elastic.booking.detail.pdf', ['id' => $purchaseOrder->id])}}">
                                                                ...
                                                                <section class="tile">
                                                                    ...
                                                                    <div class="tile-body">
                                                                        ...
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                            <div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                                                    ...
                                                                                    <select id="ProductID" class="form-control select2" name="elastic_product_setup" required style="width: 100%;" onchange="javascript:getProductPrice(this)">
                                                                            ...
                                                                            <div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                                                    <label for="GrossFormFactory" class="control-label">Gross Form Factor</label>
                                                                                    <input type="number" step="any" class="form-control GrossFormFactory" name="gross_form_factor" id="GrossFormFactory" readonly required>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                        ...
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                            <div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                                                    <label for="GrossQty" class="control-label">Gross Order Quantity</label>
                                                                                    <input type="number" step="any" class="form-control GrossQty" name="gross_order_quantity" id="GrossQty" readonly>
                                                                                </div>
                                                                            </div>
                                                                            <div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                            ...
                                                            <div class="table-responsive">
                                                                <table class="table table-hover table-bordered table-condensed table-responsive" id="advanced-usage{{$uproduct->id}}">
                                                                    ...
                                                                    <tr style="background-color: #1693A5; color: white;">
                                                                        ...
                                                                        <th class="text-center">Gross Order Quantity</th>
                                                                    ...
                                                                    <tbody>
                                                                                ...
                                                                                @foreach($details as $item)
                                                                                    @if($item->elastic_product_setup_id == $uproduct->id)
                                                                                        <tr>
                                                                                            ...
                                                                                            <td class="text-right">{{ $item->gross_order_quantity }}</td>
...
{{--                                                                                                <button class="btn btn-info btn-xs" data-toggle="modal" data-target="#user{{$item->id}}" data-options="splash-2 splash-ef-12"><i class="fa fa-eye"></i></button>--}}
                                                                                                @if(Auth::user()->hasTaskPermission('updateeitem', Auth::user()->id))
...
@section('pageScripts')
    <script>
        ...
        $('#advanced-usage{{$uproduct->id}}').on('click',".DeleteDetail", function(){
            ...
            var url = '{{ route('elastic.booking.detail.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".UrgentOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('elastic.booking.urgent') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".ReviseOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('elastic.booking.revise') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".DeleteOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('elastic.booking.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".ReturnActiveOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('elastic.booking.return-active') }}';
            swal({
                title: 'Are you sure?',
        ...
        $(function(){
            ...
            $('#ItemAdd').submit(function(e){
                ...
                //return;
                var url = '{{ route('elastic.booking.detail.save') }}';
                //console.log(data);
                $.ajax({
                    url: url,
                    method:'POST',
        ...
        function getProductPrice(_category) {
            ...
            var url = '{{ route('elastic.product.get-price') }}';
            $.ajaxSetup({
                headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}
            });
            if (categoryId) {
                $.ajax({
                    ...
                    dataType: "json",
                    success: function (data) {
                        document.forms["ItemAddForm"]["gross_form_factor"].value = data.gross_form_factor;
                        // document.forms["ItemAddForm"]["is_board"].value = data.is_board;

                    }
                });
            } else {
                document.forms["ItemAddForm"]["gross_form_factor"].value = 0;
