...
@section('content')
    ...
    <div class="page page-profile">
        ...
        <h2>QC Sticker <span> Booking Details</span></h2>
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="{{route('qcsticker.booking.recent')}}"> Recent Bookings</a>
                </li>
                <li>
                    <a href="{{route('qcsticker.booking.detail', ['id', $purchaseOrder->id ])}}"> LPD PO NO: {{$purchaseOrder->lpd_po_no}}</a>
                </li>
            </ul>
        </div>
    </div>
                            ...
                            <div class="mt-10">
                                ...
                                @else
                                    ...
                                    @if($purchaseOrder->status == 'A')
                                        @if(Auth::user()->hasTaskPermission('updateqcsmaster', Auth::user()->id))
                                            <a title="Edit Booking Master Update" class ="myIcon icon-warning icon-ef-3 icon-ef-3b icon-color" href="{{route('qcsticker.booking.edit', ['id' => $purchaseOrder->id])}}" {{--data-toggle="modal" data-target="#POUpdateModal" --}}data-options="splash-2 splash-ef-12">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                        @endif
                                        @if(Auth::user()->hasTaskPermission('reviseqcs', Auth::user()->id))
                                        ...
                                        @endif
                                        @if(Auth::user()->hasTaskPermission('deleteqcs', Auth::user()->id))
                                    ...
                                    @else
                                        ...
                                        @endif
                                    @endif
                                    <a title="PDF View" class ="myIcon icon-danger icon-ef-3 icon-ef-3b icon-color" {{--target="_blank"--}} href="{{route('qcsticker.booking.detail.pdf', ['id' => $purchaseOrder->id])}}">
                                                                        ...
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                            <div class="col-md-6 no-padding">
                                                                                <div class="form-group">
                                                                                    ...
                                                                                    <select id="ProductID" class="form-control select2" name="q_c_sticker_product_setup" required style="width: 100%;" onchange="javascript:getProductPrice(this)">
                                                                            ...
                                                                            <div class="col-md-2 no-padding">
                                                                                <div class="form-group">
                                                                                    ...
                                                                                    <input type="text" min="1" id="Color" class="form-control" name="color" required>
                                                                                </div>
                                                                            </div>
                                                                            <div class="col-md-2 no-padding">
                                                                                <div class="form-group">
                                                                                    <label for="Qty" class="control-label">SL No.</label>
                                                                                    <input type="text" min="1" id="Qty" class="form-control" name="sl_no" required>
                                                                                </div>
                                                                            </div>
                                                                            <div class="col-md-2 no-padding">
                                                                                <div class="form-group">
                                                                ...
                                                                <table class="table table-hover table-bordered table-condensed table-responsive" id="advanced-usage{{$uproduct->id}}">
                                                                    ...
                                                                    <tbody>
                                                                        ...
                                                                        @foreach($details as $item)
                                                                            @if($item->q_c_sticker_product_setup_id == $uproduct->id)
                                                                                <tr>
                                                                                    ...
                                                                                    <td class="text-center">
                                                                                        ...
                                                                                        @if(Auth::user()->hasTaskPermission('updateqcsitem', Auth::user()->id))
...
@section('pageScripts')
    <script>
        ...
        $('#advanced-usage{{$uproduct->id}}').on('click',".DeleteDetail", function(){
            ...
            var url = '{{ route('qcsticker.booking.detail.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".UrgentOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('qcsticker.booking.urgent') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".ReviseOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('qcsticker.booking.revise') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".DeleteOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('qcsticker.booking.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".ReturnActiveOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('qcsticker.booking.return-active') }}';
            swal({
                title: 'Are you sure?',
        ...
        $(function(){
            ...
            $('#ItemAdd').submit(function(e){
                ...
                //return;
                var url = '{{ route('qcsticker.booking.detail.save') }}';
                //console.log(data);
                $.ajax({
                    url: url,
                    method:'POST',
        ...
        function getProductPrice(_category) {
            ...
            var url = '{{ route('qcsticker.product.get-price') }}';
            $.ajaxSetup({
                headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}
            });
            if (categoryId) {
                $.ajax({
                    url: url,
                    data: {id: categoryId, supplier: supplier},
                    type: "POST",
                    dataType: "json",
                    success: function (data) {
        ...
        $('#ItemAdd').delegate('.UnitPrice, .Qty, .Total ','keyup',function(){
            ...
            var total_price = parseFloat((qty/12) * unit_price).toFixed(12);
