...
@section('content')
    ...
    <div class="page page-profile">
        ...
        <h2>General Item <span> Booking Details</span></h2>
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#"> General Item</a>
                </li>
                <li>
                    <a href="{{route('generalitem.booking.recent')}}"> Recent Bookings</a>
                </li>
                <li>
                    <a href="{{route('generalitem.booking.detail', ['id', $purchaseOrder->id ])}}"> LPD PO NO: {{$purchaseOrder->lpd_po_no}}</a>
                </li>
            </ul>
        </div>
    </div>
                            ...
                            <div class="mt-10">
                                ...
                                @else
                                    ...
                                    @if($purchaseOrder->status == 'A')
                                        @if(Auth::user()->hasTaskPermission('updategimaster', Auth::user()->id))
                                            <a title="Edit Booking Master Update" class ="myIcon icon-warning icon-ef-3 icon-ef-3b icon-color" href="{{route('generalitem.booking.edit', ['id' => $purchaseOrder->id])}}" {{--data-toggle="modal" data-target="#POUpdateModal" --}}data-options="splash-2 splash-ef-12">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                        @endif
                                        @if(Auth::user()->hasTaskPermission('revisegi', Auth::user()->id))
                                        ...
                                        @endif
                                        @if(Auth::user()->hasTaskPermission('deletegi', Auth::user()->id))
                                    ...
                                    @else
                                        ...
                                        @endif
                                    @endif
                                    <a title="PDF View" class ="myIcon icon-danger icon-ef-3 icon-ef-3b icon-color" {{--target="_blank"--}} href="{{route('generalitem.booking.detail.pdf', ['id' => $purchaseOrder->id])}}">
                                                                        ...
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                            <div class="col-md-8 no-padding">
                                                                                <div class="form-group">
                                                                                    ...
                                                                                    <select id="ProductID" class="form-control select2" name="general_product_setup" required style="width: 100%;" onchange="javascript:getProductPrice(this)">
                                                                        ...
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                            ...
                                                                            <div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                                                    ...
                                                                                    <input type="number" step="any" id="Total" class="form-control Total" name="total_price" required>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                ...
                                                                <table class="table table-hover table-bordered table-condensed table-responsive" id="advanced-usage{{$uproduct->id}}">
                                                                    ...
                                                                    <tbody>
                                                                        ...
                                                                        @foreach($details as $item)
                                                                            @if($item->general_product_setup_id == $uproduct->id)
                                                                                <tr>
                                                                                    ...
                                                                                    <td class="text-center">
                                                                                        ...
                                                                                        @if(Auth::user()->hasTaskPermission('updategiitem', Auth::user()->id))
...
@section('pageScripts')
    <script>
        ...
        $('#advanced-usage{{$uproduct->id}}').on('click',".DeleteDetail", function(){
            ...
            var url = '{{ route('generalitem.booking.detail.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".UrgentOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('generalitem.booking.urgent') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".ReviseOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('generalitem.booking.revise') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".DeleteOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('generalitem.booking.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".ReturnActiveOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('generalitem.booking.return-active') }}';
            swal({
                title: 'Are you sure?',
        ...
        $(function(){
            ...
            $('#ItemAdd').submit(function(e){
                ...
                //return;
                var url = '{{ route('generalitem.booking.detail.save') }}';
                //console.log(data);
                $.ajax({
                    url: url,
                    method:'POST',
        ...
        function getProductPrice(_category) {
            ...
            var url = '{{ route('generalitem.product.get-price') }}';
            $.ajaxSetup({
                headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}
            });
            if (categoryId) {
        ...
        $('#ItemAdd').delegate('.UnitPrice, .Qty, .Total ','keyup',function(){
            ...
            num_total_b = upto3Decimal(total_sqm_price_b); //With 3 exposing the hundredths place
