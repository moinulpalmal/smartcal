...
@section('content')
    ...
    <div class="page page-profile">
        ...
        <h2>Tissue <span> Booking Details</span></h2>
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="{{route('tissue.booking.recent')}}"> Recent Bookings</a>
                </li>
                <li>
                    <a href="{{route('tissue.booking.detail', ['id', $purchaseOrder->id ])}}"> LPD PO NO: {{$purchaseOrder->lpd_po_no}}</a>
                </li>
            </ul>
        </div>
    </div>
                            ...
                            <div class="mt-10">
                                ...
                                @else
                                    ...
                                    @if($purchaseOrder->status == 'A')
                                        @if(Auth::user()->hasTaskPermission('updatetmaster', Auth::user()->id))
                                            <a title="Edit Booking Master Update" class ="myIcon icon-warning icon-ef-3 icon-ef-3b icon-color" href="{{route('tissue.booking.edit', ['id' => $purchaseOrder->id])}}" {{--data-toggle="modal" data-target="#POUpdateModal" --}}data-options="splash-2 splash-ef-12">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                        @endif
                                        @if(Auth::user()->hasTaskPermission('reviset', Auth::user()->id))
                                        ...
                                        @endif
                                        @if(Auth::user()->hasTaskPermission('deletet', Auth::user()->id))
                                    ...
                                    @else
                                        ...
                                        @endif
                                    @endif
                                    <a title="PDF View" class ="myIcon icon-danger icon-ef-3 icon-ef-3b icon-color" {{--target="_blank"--}} href="{{route('tissue.booking.detail.pdf', ['id' => $purchaseOrder->id])}}">
                                                                        ...
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                            <div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                                                    ...
                                                                                    <select id="ProductID" class="form-control select2" name="tissue_product_setup" required style="width: 100%;" onchange="javascript:getProductPrice(this)">
                                                                            ...
                                                                            <div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                                                    <label for="GrossFormFactory" class="control-label">DZ Conversion Value</label>
                                                                                    <input type="number" step="any" class="form-control GrossFormFactory" name="gross_form_factor" id="GrossFormFactory" value="12" readonly required>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                            <div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                                                    <label for="Size" class="control-label">Item Size</label>
                                                                                    <input type="text" class="form-control" name="size" id="Size" required>
                                                                                </div>
                                                                            </div>
                                                                            {{--<div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                                                    <label for="Color" class="control-label">Item Color</label>
                                                                            ...
                                                                            <div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                                                    <label for="GrossQty" class="control-label">Order Quantity in DZ</label>
                                                                                    <input type="number" step="any" class="form-control GrossQty" name="order_quantity_dz" id="GrossQty" readonly>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                            <div class="col-md-6 no-padding">
                                                                                <div class="form-group">
                                                                                    <label for="UnitPrice" class="control-label">Per DZ Unit Price (USD)</label>
                                                                                    <input type="number" id="UnitPrice" class="form-control UnitPrice" step="any" name="per_dz_price" required>
                                                                                </div>
                                                                            </div>
                                                                            <div class="col-md-6 no-padding">
                                                                                <div class="form-group">
...
{{--                                                                        <th class="text-center">Color</th>--}}
                                                                        ...
                                                                        <th class="text-center">Order Quantity (DZ)</th>
                                                                        <th class="text-center">Per DZ Unit Price (USD)</th>
                                                                    ...
                                                                    <tbody>
                                                                                ...
                                                                                @foreach($details as $item)
                                                                                    @if($item->tissue_product_setup_id == $uproduct->id)
                                                                                        <tr>
...
{{--                                                                                            <td class="text-left">{{$item->color}}</td>--}}
                                                                                            ...
                                                                                            <td class="text-right">{{ $item->order_quantity_dz }}</td>
                                                                                            <td class="text-right">$ {{ $item->per_dz_price }}</td>
...
{{--                                                                                                <button class="btn btn-info btn-xs" data-toggle="modal" data-target="#user{{$item->id}}" data-options="splash-2 splash-ef-12"><i class="fa fa-eye"></i></button>--}}
                                                                                                @if(Auth::user()->hasTaskPermission('updatetitem', Auth::user()->id))
...
@section('pageScripts')
    <script>
        ...
        $('#advanced-usage{{$uproduct->id}}').on('click',".DeleteDetail", function(){
            ...
            var url = '{{ route('tissue.booking.detail.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".UrgentOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('tissue.booking.urgent') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".ReviseOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('tissue.booking.revise') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".DeleteOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('tissue.booking.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".ReturnActiveOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('tissue.booking.return-active') }}';
            swal({
                title: 'Are you sure?',
        ...
        $(function(){
            ...
            $('#ItemAdd').submit(function(e){
                ...
                //return;
                var url = '{{ route('tissue.booking.detail.save') }}';
                //console.log(data);
                $.ajax({
                    url: url,
                    method:'POST',
        ...
        function getProductPrice(_category) {
            ...
            //console.log("hit");
            var url = '{{ route('tissue.product.get-price') }}';
            $.ajaxSetup({
                headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}
            });
            if (categoryId) {
                $.ajax({
                    url: url,
                    data: {id: categoryId, supplier_id: supplierId},
                    type: "POST",
                    dataType: "json",
                    success: function (data) {
                        //console.log(data);
                        //return;
                        document.forms["ItemAddForm"]["per_dz_price"].value = data.per_dz_price;
                        // document.forms["ItemAddForm"]["is_board"].value = data.is_board;

                    }
                });
            } else {
                document.forms["ItemAddForm"]["per_dz_price"].value = 0;
        ...
        $('#ItemAdd').delegate('.GrossFormFactory, .UnitPrice, .Qty, .GrossQty, .Total ','keyup',function(){
            var tr = $(this).parent().parent().parent().parent().parent().parent();

            var grossFormFactory = parseFloat(tr.find('.GrossFormFactory').val()).toFixed(12);
            //console.log(grossFormFactory);
            var qty = parseInt(tr.find('.Qty').val());
            //console.log(qty);

            var gross_qty = parseFloat(qty/grossFormFactory).toFixed(12);
            tr.find('.GrossQty').val(gross_qty);
            //console.log(gross_qty);
            //return;
            var unit_price = parseFloat(tr.find('.UnitPrice').val()).toFixed(12);
            var total_price = parseFloat(gross_qty * unit_price).toFixed(12);
