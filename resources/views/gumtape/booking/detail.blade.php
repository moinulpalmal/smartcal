...
@section('content')
    ...
    <div class="page page-profile">
        ...
        <h2>Gum Tape <span> Booking Details</span></h2>
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="{{route('gumtape.booking.recent')}}"> Recent Bookings</a>
                </li>
                <li>
                    <a href="{{route('gumtape.booking.detail', ['id', $purchaseOrder->id ])}}"> LPD PO NO: {{$purchaseOrder->lpd_po_no}}</a>
                </li>
            </ul>
        </div>
    </div>
                            ...
                            <div class="mt-10">
                                ...
                                @else
                                    ...
                                    @if($purchaseOrder->status == 'A')
                                        @if(Auth::user()->hasTaskPermission('updategtmaster', Auth::user()->id))
                                            <a title="Edit Booking Master Update" class ="myIcon icon-warning icon-ef-3 icon-ef-3b icon-color" href="{{route('gumtape.booking.edit', ['id' => $purchaseOrder->id])}}" {{--data-toggle="modal" data-target="#POUpdateModal" --}}data-options="splash-2 splash-ef-12">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                        @endif
                                        @if(Auth::user()->hasTaskPermission('revisegt', Auth::user()->id))
                                        ...
                                        @endif
                                        @if(Auth::user()->hasTaskPermission('deletegt', Auth::user()->id))
                                    ...
                                    @else
                                        ...
                                        @endif
                                    @endif
                                    <a title="PDF View" class ="myIcon icon-danger icon-ef-3 icon-ef-3b icon-color" {{--target="_blank"--}} href="{{route('gumtape.booking.detail.pdf', ['id' => $purchaseOrder->id])}}">
                                                                ...
                                                                <section class="tile">
                                                                    ...
                                                                    <div class="tile-body">
                                                                        ...
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                            <div class="col-md-4 no-padding">
                                                                                <div class="form-group">
                                                                                    ...
                                                                                    <select id="ProductID" class="form-control select2" name="gum_tape_product_setup" required style="width: 100%;">
                                                                        ...
                                                                        <div class="row" style="padding: 0px 15px;">
                                                                            ...
                                                                            <div class="col-md-3 no-padding">
                                                                                <div class="form-group">
                                                                                    <label for="CurrencyID" class="control-label">Currency</label>
                                                                                    <select id="CurrencyID" class="form-control select2" name="currency" required style="width: 100%;" >
                                                                                        <option value="">- - - Select - - -</option>
                                                                ...
                                                                <table class="table table-hover table-bordered table-condensed table-responsive" id="advanced-usage{{$uproduct->id}}">
                                                                    ...
                                                                    <tbody>
                                                                        ...
                                                                        @foreach($details as $item)
                                                                            @if($item->gum_tape_product_setup_id == $uproduct->id)
                                                                                <tr>
                                                                                    ...
                                                                                    <td class="text-center">
                                                                                        ...
                                                                                        @if(Auth::user()->hasTaskPermission('updategtitem', Auth::user()->id))
...
@section('pageScripts')
    <script>
        ...
        $('#advanced-usage{{$uproduct->id}}').on('click',".DeleteDetail", function(){
            ...
            var url = '{{ route('gumtape.booking.detail.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".UrgentOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('gumtape.booking.urgent') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".ReviseOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('gumtape.booking.revise') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".DeleteOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('gumtape.booking.delete') }}';
            swal({
                title: 'Are you sure?',
        ...
        $('#purchase-order').on('click',".ReturnActiveOrder", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('gumtape.booking.return-active') }}';
            swal({
                title: 'Are you sure?',
        ...
        $(function(){
            ...
            $('#ItemAdd').submit(function(e){
                ...
                //return;
                var url = '{{ route('gumtape.booking.detail.save') }}';
                //console.log(data);
                $.ajax({
                    url: url,
                    method:'POST',
        ...
        $('#ItemAdd').delegate('.UnitPrice, .Qty, .Total ','keyup',function(){
            ...
            //num_total_b = num_total_b.slice(0, (num_total_b.indexOf("."))+3); //With 3 exposing the hundredths place
            var num_total_b = upto2Decimal(total_sqm_price_b);
